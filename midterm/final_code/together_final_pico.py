...
class MusicBox:
    ...
    async def get_distance(self):
        ...
        self.sig.low()
        await asyncio.sleep(0.02)  # Ensure pulse start
        self.sig.high()
        await asyncio.sleep(0.00001)  # Short 10us pulse duration
        self.sig.low()

        self.sig.init(Pin.IN)

        # Measure pulse duration using async polling
        pulse_start = None
        pulse_end = None
        ...
        # Record the start time (based on asyncio event loop)
        ...
        # Convert pulse duration to distance in centimeters (speed of sound = 34300 cm/s)
